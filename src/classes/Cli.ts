  ...
  createTruck(): void {
    inquirer
      ...
      .then((answers) => {
        ...
        const wheels = new Array(4).fill(
          new Wheel(answers.wheelDiameter, answers.wheelBrand)
        );
        const truck = new Truck(Cli.generateVin(), // Use Cli.generateVin() for VIN
          ...
          parseInt(answers.topSpeed),
          wheels,
          parseInt(answers.towingCapacity) // Ensure towingCapacity is passed as an argument
        );
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        const motorbike = new Motorbike(
          Cli.generateVin(), // Use Cli.generateVin() for VIN
          answers.color,
          answers.make,
          answers.model,
          parseInt(answers.year),
          parseInt(answers.weight),
          parseInt(answers.topSpeed),
          [frontWheel, rearWheel] // Pass the front and rear wheels as an array
        );
